...
if ($result['success'] && (isset($_GET['reportType']) && $_GET['reportType']) && file_exists(APP_ROOT_V3.'/uploads/')) {
    $report = $uploader->copy(APP_ROOT.'/uploads/'.$result['filename'], APP_ROOT_V3.'/uploads/'.$result['filename']);
    if (!$report) {
       return array('Unable to upload in v2 upload directory check APP_ROOT_V2 constant in config.php file'); 
    }
