...
class panelRequestManager{
	...
	public static function handler($requestData, $fromAjax = false){
		...
		if(manageClients::methodExists($action)){
			$getSlowDownAjaxCallFromCookie = manageCookies::cookieGet('slowDownAjaxCallFrom');
			if (!empty($getSlowDownAjaxCallFromCookie)) {
		...
		if(empty($requestData['noGeneralCheck']) && $fromAjax){
			if (defined('APP_V3')) {
				panelRequestManagerV3::generalCheckV3($finalResponse);
			}else{
	...
	public static function getBackupsByTaskName($siteID, $taskName, $isNewBackup = false, $backupID = false){
		if($refresh){
			manageClients::getStatsProcessor(array($siteID));
		}
		...
		$sitesStatRaw = DB::getRow("?:site_stats", "*", $where);
		$sitesStat = unserialize(base64_decode($sitesStatRaw['stats']));
		if ($isNewBackup) {
			$backups = $sitesStat['iwp_new_backups'][$taskName];
		}else{
			$backups = $sitesStat['iwp_backups'][$taskName];
		}
		if ($backupID != false) {
	...
	public static function getNewBackupDetails($params){
		$backupDetails = self::getBackupFilesByTaskName($params['siteId'], $params['taskName'], true, $params['backupID']);
		$params['backupDetails'] = $backupDetails;
		return $params;
	}

	public static function getBackupFilesByTaskName($siteID, $taskName, $isNewBackup = false, $backupID = false){
		$backupFiles = array();
		$backupDetails = self::getBackupsByTaskName($siteID, $taskName, $isNewBackup, $backupID);
	...
	public static function getRawSitesStats($siteIDs=array(), $userID = '', $doCache = true){
		...
		if(function_exists('multiUserRawSitesStats')){
			...
			$sitesStats = DB::getArray("?:site_stats SS, ?:sites S", "SS.*", "S.siteID = SS.siteID AND SS.stats IS NOT NULL AND S.type = 'normal' ".$where.$ORDER, "siteID");
		}
	...
	public static function getSitesBackups($siteIDs=array(), $isClone = false, $isSingleSite = false){
		...
		foreach($sitesStats as $siteID => $siteStats){
			if (isChildSite($siteID)) {
				...
				if ($parentSiteID == false) {
					...
					if ($isSingleSite == false) {
						$siteStats = $sitesStats[$parentSiteID];
					}else{
			...
			$backupKeys = array();
			if (!empty($siteStats['stats']['iwp_backups'])) {
				...
				if (!empty($siteStats['stats']['iwp_new_backups'])) {
					...
					if (!empty($siteBackupsTemp)) {
						...
						foreach ($siteBackupsTemp as $referenceKey => $siteBackupTemp) {
							...
							$fileURLParts = '';
							if ((!empty($siteBackupTemp['plugins']) || !empty($siteBackupTemp['themes']) || !empty($siteBackupTemp['uploads']) || !empty($siteBackupTemp['others']) ) && !empty($siteBackupTemp['db'])) {
								$what = 'full';
							...
							if ($isClone) {
								$backupFileBasename = $siteBackupTemp['wp_content_url'].'/infinitewp/backups/'.$siteBackupTemp['backup_file_basename'];
							}else{
							...
							$sitesBackups[$siteID][$backupTaskType][$referenceKey] = array('backupName' 	=> $siteBackupTemp['label'],
																								'sitesCount'	=> $count,
				...
				if (!empty($siteStats['stats']['iwp_backups']) && $parentSiteID == false){
					...
					if (!empty($siteBackupsTemp)){ 
						...
						foreach($siteBackupsTemp as $referenceKey => $siteBackupTemp){
							...
							$sitesBackups[$siteID][$backupTaskType][$referenceKey] = array('time' => $siteBackupTemp['time'],
																		  'type' => 'backupNow',
	...
	public static function getSiteBackupsHTML($siteID){
		$sitesBackups['manualBackups'] = self::getSitesBackups(array($siteID), false, true);
	...
	public static function getSitesUpdates($userID = '', $doCache = true, $siteIDs = array()){
		...
		$siteView = $pluginView = $themeView = $coreView = $translationsView = $wooDBUpdate = array();
		...
		foreach($sitesStats as $siteID){
			...
			$siteID['stats']['additional_updates'] = (array)$siteID['stats']['additional_updates'];

			if (!empty($siteID['stats']['additional_updates'])) {
				foreach ($siteID['stats']['additional_updates'] as $item) {
					$wooDBUpdate[$siteID['siteID']] = true;
					$wooDBUpdateCount++;
				}
			}
	...
	public static function getSites(){
		...
		if(function_exists('multiUserGetSites')){
			...
			$sitesData = DB::getArray("?:sites", "siteID, URL, adminURL, name, IP, WPVersion, adminUsername, isOpenSSLActive, network, parent, httpAuth, callOpt, connectURL, connectionStatus, links, notes, favicon, lastFaviconUpdate, stagingFtpDetails, ftpDetails, customBranding", "type = 'normal' ORDER BY name", "siteID");
		}
	...
	public static function updateSettings($settings){
		...
		if(!empty($settings['general'])){
			...
			if(!empty($currentGeneralSettings['enableSSLVerify'])){
				$settings['general']['enableSSLVerify'] = $currentGeneralSettings['enableSSLVerify'];
			}
	...
	public static function getActionStatus($actionID, $action='', $skipDetailedAction = false){
		...
		if ($actionType == 'PTC') {
			$updateWhere = ' GROUP BY siteID ';
		}
		$where = array(
		      		'query' =>  "actionID = ':actionID' ".$updateWhere."ORDER BY historyID ASC",
		      		'params' => array(
		               ':actionID'=>$actionID
	   				)
				);
	...
	public static function getWPRepositoryHTML($params){
		...
		foreach($return as $item)
		{
			...
			if($type=='plugins')
			{
				...
				$content = $content.'<a class="addToFavorites" utype="'.$type.'" iname="'.$item->name.'" islug="'.$item->slug.'" dlink="'.$item->download_link.'" style="position:relative">Add to Favorites</a>';
			...
			$content=$content.'	<a class="addToFavorites" utype="'.$type.'" iname="'.$item->name.'" islug="'.$item->slug.'" dlink="http://wordpress.org/themes/download/'.$item->slug.'.'.$item->version.'.zip" style="position:relative">Add to Favorites</a>';
			$content = $content.'</div>
                <div class="clear-both"></div>
	...
	public static function updatesNotificationMailTest(){
		// params1 is used to testconnection email,params 2 used to Update Notification 
		return updatesNotificationMailSend(true,false);
	}
	...
	public static function generalCheck(&$finalResponse){
		...
		if($updateAvailable = checkUpdate()){
			...
			$updateAvailable = getCachedV3UpdateDetails();
			if( getOption('updateHideNotify') != $updateAvailable['newVersion'] && getOption('updateNotifySentToJSV3') != $updateAvailable['newVersion'] ){
				$finalResponse['updateAvailableV3'] = $updateAvailable;
				updateOption('updateNotifySentToJSV3', $updateAvailable['newVersion']);
			}
		}
	...
	public static function getResponseCurlVerbose($historyID){
		$options = array();
		$where = array(
		...
		$callInfo = DB::getField("?:history_raw_details", "callInfo", $where);
		$callInfoData = unserialize($callInfo);
		$historyDetails = DB::getArray("?:history", "siteID, URL",$where);
		if(empty($historyDetails) && empty($callInfoData)){
			return 'Empty Response';
		}
		if(!empty($historyDetails[0])){
			$historyData = $historyDetails[0];
		}
		if(!empty($callInfoData['info']) && !empty($callInfoData['info']['url'])){
			$url = $callInfoData['info']['url'];
		}else{
			$url = $historyData['URL'];
		}
		if(!empty($historyData['siteID'])){
			$siteID = $historyData['siteID'];
			$byPassAccess = true;
			$siteDetails = getSiteData($siteID,"1",$byPassAccess);
			if(!empty($siteDetails['callOpt'])){
				$callOpt = unserialize($siteDetails['callOpt']);
				if(!empty($callOpt)){
					$options['HTTPVersion'] = $callOpt['HTTPVersion'];
					$options['contentType'] = $callOpt['contentType'];
					$options['SSLVersion']  = $callOpt['SSLVersion'];
				}
			}
			if(!empty($siteDetails['httpAuth'])){
				$httpAuth = unserialize($siteDetails['httpAuth']);
				$options['httpAuth']['username'] = $httpAuth['username'];
				$options['httpAuth']['password'] = $httpAuth['password'];
			}
			if(!empty($siteDetails['siteCookie'])){
				$options['useCookie'] = $siteDetails['siteCookie'];
			}
		}

		$data = array('curlVerboseFromPanel'=>true);
		$timeout = 300;
		return doCall($url,$data,$timeout,$options);
	}
	...
	public static function getSystemCronRunningFrequency($data = array()){
		if (is_array($data) && $data['bothCheck'] == true) {
			return getSystemCronRunningFrequency(true);
		}
	...
	public static function terminatePendingProcess($params){
		...
		if($condition != ""){
			...
			$historyDatas = DB::getArray("?:history", "historyID, siteID, type", $condition." AND status NOT IN ('completed','error','netError') ");
			...
			if(!empty($historyDatas) && is_array($historyDatas)){
				foreach($historyDatas as $key => $historyData){
					...
					if($params["multiCall"]){
						...
						$action = DB::getField("?:history", "action", $where);
						if (in_array($action, array('restoreBackupDownlaod', 'multiCallRestore', 'restoreNew')) ) {
							continue;
						}
						...
						if ($parentHistoryRequest['params']['mechanism'] == 'advancedBackup') {
							$requestAction = 'kill_new_backup';
							$where = array(
					      		'query' =>  "parentHistoryID=':historyID' AND status = 'scheduled'",
					      		'params' => array(
					               ':historyID'=>$historyData['historyID']
									)
							);
						$hisID = DB::getField("?:history", "historyID", $where);
						$resultID = $parentHistoryRequest['params'] ['backup_nounce'];
						updateHistory(array("status" => "error", "error" => "task_killed"), $hisID, array("status" => "error", "errorMsg" => "Task killed by user"));

						}else{
							$requestAction = 'delete_backup';
							$resultID = $historyData['historyID'];
						}

						$historyAdditionalData[] = array('uniqueName' => "Backup Now", 'detailedAction' => "remove");
						$PRP = array('requestAction' => $requestAction,'requestParams' => array( 'task_name' => "Backup Now",'result_id' => $resultID),'siteData' => getSiteData($historyData['siteID']),'type' => "backup",'action' => "remove",'status' => "pending",'events' => 1,'historyAdditionalData' => $historyAdditionalData,'doNotShowUser' => true,'sendAfterAllLoad' => false/*,'timeScheduled' => time()+30*/);
						prepareRequestAndAddHistory($PRP);
					}elseif ($historyData['type'] == 'backup' || $historyData['type'] == 'scheduleBackup') {
	...
	public static function getReaddedSite(){
		$actionID = Reg::get('currentRequest.actionID');
		$where = array(
		      		'query' =>  "actionID = ':actionID'",
		      		'params' => array(
	...
	public static function iwpMaintenance(){
		$actionID = Reg::get('currentRequest.actionID');
		$where = array(
		      		'query' =>  "actionID = ':actionID'",
		      		'params' => array(
	...
	public static function clearHistoryByIDs($historyIDs = array() ){
		if(!empty($historyIDs)){
			...
			if (defined('IWP_DISABLE_RAW_TABLE_OPTIMIZE') && IWP_DISABLE_RAW_TABLE_OPTIMIZE) {
				return true;
			}
    	...
    	if(isset($params) && isset($params['clearWhat'])){
    		if($params['clearWhat'] == 'uncomplete'){
				...
				if(!empty($params['time'])){
					...
					$historyIDCount = DB::getField("?:history", "count(historyID)", $where.$accessUsers );
					$limitSQL = '';
					if ($historyIDCount >= '2000') {
						$limitSQL = ' LIMIT 2000';
						$moreHistory = true;
					}
					$historyIDs = DB::getFields("?:history", "historyID", $where.$accessUsers.$limitSQL );
					self::clearHistoryByIDs($historyIDs);
					// if(!empty($historyIDs)){
					// 	$historyIDs = implode("','", $historyIDs);
					// 	DB::delete("?:history", "historyID IN ('".$historyIDs."')");
	...
	public static function autoCheckAndDeleteHistoryRawLog($params){
		$last = getOption('LastAutoDeleteRawLogTime');
		if(!empty($last)){
			$now = time();
			if(date("DDMMYY",$last) != date("DDMMYY",$now) && $last < $now){
				$days = 30;
				...
				$delParams['time'] = $toDelete;
				$delParams['LastAutoDeleteLogTime'] = $now;
				self::clearHistoryRawTable($delParams);
			}
		}else{
			updateOption('LastAutoDeleteRawLogTime', 1);
		}
	}

	public static function clearHistoryRawTable($params){
		if(!empty($params['time'])){
			$where = " microtimeAdded <= '".DB::esc($params['time'])."' ";
			$accessUsers = " ";
			setHook('historyHTML', $accessUsers);
			...
			updateOption('LastAutoDeleteRawLogTime', $now);
		}
	}
	...
	public static function getActivityCategories($params){
		...
		$activityCatsAddon = array(
								...
								'Wordfence' => 'wordFence',
								'WP_Optimize' => 'wp',
								'Staging' => array('staging', 'stagingToLive')
							);
		//nameConversions
		$activityName = array(
	...
	private static function checkDataIsValid($action){
		...
		$functions = array('addFunctions', 'addfunctions');
	...
	public static function addClassToPanelRequest($className){
        ...
        public static function getSiteFtpDetails($params){
        	...
        	$updateResult = DB::getRow("?:sites", "ftpDetails", $where);
        	if($updateResult === false){
        		return false;
        	}
        	return unserialize($updateResult['ftpDetails']);
        }

        public static function getSiteFtpDetailsClone($params){
        	if(empty($params['siteID'])){
        		return false;
        	}
        ...
        public static function saveSiteFtpDetails($params){
        	if(empty($params)){
        		return false;
        	}
        	if(!empty($params['siteID'])){
        		$siteData = array("ftpDetails" => serialize($params));
        		$where = array(
        			'query' =>   "siteID=':siteID'",
        				'params' => array(
        					':siteID' => $params['siteID'],
        		...
        		$updateResult = DB::update("?:sites", $siteData, $where);
        	}
        	return $updateResult;
        }

        public static function manualRetryFailedTask($historyID){
        	updateHistory(array('status' => "pending", 'error' => NULL, 'isV3' => 0,'param3'=>'manualRetry'), $historyID);
        	$where = array(
		      		'query' =>  "historyID=:historyID",
		      		'params' => array(
		               ':historyID'=>$historyID
        ...
        public static function manualRetryALLFailedTask($actionID){
        	$failedTasks = DB::getFields("?:history_additional_data HAD, ?:history H", "HAD.historyID", "H.actionID = '".$actionID."' AND HAD.historyID = H.historyID AND HAD.status IN('error','netError')");
        	if (empty($failedTasks)) {
        		return false;
        	}
        	foreach ($failedTasks as $key => $historyID) {
        		self::manualRetryFailedTask($historyID);
        	}
        }

        public static function getCachedV3UpdateDetails(){
        	return getCachedV3UpdateDetails();
        }
        public static function getMultisitesByParentSiteURL($parmas){
        	$where = array(
        		      		'query' =>  "adminURL=':adminURL'",
        		      		'params' => array(
        		               ':adminURL'=>$parmas['URL']
        						)
        				);
        	$parentSiteID = DB::getField("?:sites", "siteID", $where);
        	$sites = getAllNetworkSitesByParentSiteID($parentSiteID);
        	if (empty($sites)) {
        		return false;
        	}
        	return array('parentSiteID' =>$parentSiteID , 'sites' => $sites);
        }
        public static function addBulkNetworkSite($params){
        	if (empty($params['selectedSites'])) {
        		return false;
        	}
        	$parentSiteID = $params['parentSiteID'];
        	foreach ($params['selectedSites'] as $key => $site) {
        		$addSiteParams = array('URL' => $site, 'iwp_admin_version' => '2.10.1', 'isNetworkSite' => 1, 'parentSiteID' => $parentSiteID);
        			$data = array('action' => 'addSite', 
        				'args' => array(
       	...
       	public static function getWooDBUpdateSites($params){
       		$sitesUpdates = panelRequestManager::getSitesUpdates();
       		$wooDBUpdate = $sitesUpdates['wooDBUpdate'];
       		$sitesDetails = array();
       		foreach ($wooDBUpdate as $siteID => $value) {
       			$where = array(
        			'query' =>   "siteID=':siteID'",
        				'params' => array(
        					':siteID' => $siteID,
       			...
       			$name = DB::getFields('?:sites', 'name', $where);
       			$sitesDetails[$siteID] = $name;
       		}
       		return $sitesDetails;
       	}

       	public static function getServicePopupNotification(){
       		// return false;
       		// 0 - v2 , 1 - v3 , 2 - both
       		if (defined('APP_V3')) {
				$version = "(version = '1' OR version = '2')";	
			}else{
				$version = "(version = '0' OR version = '2')";	
			}
       		$where = array(
       		      		'query' =>  "nextSchedule <= ':time' AND status = '1' AND isShowed = '0' AND".$version."ORDER BY nextSchedule",
       		      		'params' => array(
       		               ':time'=>time()
       		...
       		$notification = DB::getArray("?:popup_notification", "*", $where);
       		...
       		foreach ($notification as $key => $value) {
       			if ($counter == 0) {
       				...
       				$nextSchedule = time()+$value['breakTime'];
       			}
       	...
       	public static function closePopupNotification($data){
       		$where = array(
       				      		'query' =>  "ID = ':ID'",
       				      		'params' => array(
       				               ':ID'=>$data['ID']
       		...
       		$oneTimeOnly = DB::getField("?:popup_notification", "oneTimeOnly", $where);

       		if (!empty($oneTimeOnly) && $oneTimeOnly == '1') {
       			$insertAry['isShowed'] = 1;
       			DB::update('?:popup_notification', $insertAry, $where);
       		}
       	}

       	public static function isAddonPlanLimitExceededAttemptDetails() {//New pricing plan
       		$addonsDetails['isExceeded'] = self::checkIsAddonPlanLimitExceeded();
       		$addonsDetails['addSiteHTML'] = getAddSiteHTML();
       		return $addonsDetails;
       	}
       	public static function addonSuiteLimitExceededDetails() {//New pricing plan
       		$addonsDetails['addonSuiteLimitExceededAttempt'] = self::checkIsAddonPlanLimitExceeded('addonSuiteLimitExceededIllegally');
       		$addonsDetails['illegalHTML'] = getAddonsIllegalHTML();
       		
       		return $addonsDetails;
       	}

       	public static function iscompletedAddonsInstallation(){
       		$option = getOption('infinitewpAccountCredentials');
       		if (empty($option)) {
       			return "true";
       		}
       	...
       	public static function getInfinitewpAccountCredentials(){
       		$option = getOption('infinitewpAccountCredentials');
       		if (empty($option)) {
       			return "false";
       		}
       	...
       	public static function isNewAddonsAvailable(){
		...
		public static function notifyIfBothPanelCronRun(){
			$lastNotify = getoption('cronClashNotification');
			...
			updateOption('cronClashNotification', time());
			if ($v2Result > 20 || $v3Result > 20 || $v3Result <= 0 || $v2Result <= 0 ) {
				return false;
			}
			addNotification($type='E', $title='Multiple cronjobs detected.', $message='Looks like cronjobs are running for both v2 and v3 dashboard. Kindly disable/delete one.', $state='U', $callbackOnClose='', $callbackReference='');
			updateOption('cronClashNotification', time());

		}

		public static function onBrowserLoad(){
			onBrowserLoad();
		}
