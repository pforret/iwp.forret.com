 ...
 <?php    $inProgress=0;
          if(in_array($actionHistory['status'], array('pending', 'running', 'initiated', 'processingResponse','multiCallWaiting'))){$inProgress=1;} ?>
            ...
            <div class="rh <?php if(in_array($actionHistory['status'], array('pending', 'running', 'initiated', 'processingResponse', 'multiCallWaiting'))){ ?> in_progress<?php } ?>">
                  ...
                  <?php if($errorCount = ($actionHistory['statusSummary']['error'] + $actionHistory['statusSummary']['netError'])) { ?>
                  <a class="btn_send_report float-right needConfirm droid400 retryAllFailure" actionid="<?php echo $actionID; ?>">Retry Failed</a>
                  <?php } ?>
                  <div class="row_name"><?php echo $TPLPrepareHistoryBriefTitle; ?></div>
                  <?php if($actionHistory['statusSummary']['success']) { ?><div class="success_bu n rep_sprite_backup" style="position:relative"><?php echo $actionHistory['statusSummary']['success']; ?></div><?php } ?>
                  <?php if($errorCount = ($actionHistory['statusSummary']['error'] + $actionHistory['statusSummary']['netError'])) { ?><div class="failed_bu e rep_sprite_backup" style="position:relative"><?php echo $errorCount; ?></div><?php } ?>
...
foreach($actionHistory['detailedStatus'] as $singleAction){
  if(isset($singleAction['isV3'])){
    if($singleAction['isV3'] == 1){
      $version = 'v3';
    }elseif($singleAction['isV3'] == 0){
      $version = 'v2';
    }else{
      $version = '';
    }
    ...
    $fullGroupedActions[ $singleAction['siteID'] ][ $singleAction['detailedAction'] ][ 'success' ] [] = array('name' => $singleAction['uniqueName'],'detailedAction' => $singleAction['detailedAction'],'type' => $actionHistory['type'], 'action' => $actionHistory['action'], 'successMsg' => $singleAction['successMsg'],'historyID'=> $singleAction['historyID'] );
		
	}
	elseif($singleAction['status'] == 'error' || $singleAction['status'] == 'netError'){	
		
		$fullGroupedActions[ $singleAction['siteID'] ][ $singleAction['detailedAction'] ][ 'error' ] [] = array('name' => $singleAction['uniqueName'], 'errorMsg' => $singleAction['errorMsg'], 'error' => $singleAction['error'], 'type' => $actionHistory['type'], 'action' => $actionHistory['action'], 'detailedAction' => $singleAction['detailedAction'], 'microtimeInitiated' => $singleAction['microtimeInitiated'], 'status' => $singleAction['status'],'historyID'=> $singleAction['historyID']);
		$siteWithErrors[$singleAction['siteID']] = $singleAction['historyID'];
	}
	else{
    $fullGroupedActions[ $singleAction['siteID'] ][ $singleAction['detailedAction'] ][ 'others' ] [] = array('name' => $singleAction['uniqueName'], 'errorMsg' => $singleAction['mainStatus'], 'microtimeInitiated' => $singleAction['microtimeInitiated'], 'status' => $singleAction['status'], 'successMsg' => $singleAction['successMsg'], 'historyID'=> $singleAction['historyID']);
  ...
  $sitesDataTemp[$singleAction['siteID']]['version'] = $version;
}
?>
<?php foreach($fullGroupedActions as $siteID => $siteGroupedActions){ ?>   
 <!--           For each Site starts-->
                ...
                <div class="row_updatee_ind">
                    ...
                    <div class="label droid700 float-left version_number"><?php if(!empty($siteWithErrors[$siteID])){ ?><a style="float:left; position:absolute; left:174px; bottom:2px;font-size: 10px;" class="moreInfo "  historyID="<?php echo $siteWithErrors[$siteID]; ?>">View site response</a><?php unset($siteWithErrors[$siteID]); } ?><?php echo $sitesDataTemp[$siteID]['name'].'-<span>'.$sitesDataTemp[$siteID]['version'].'</span>'; ?></div>
                    <div class="clear-both"></div>
                  </div>
                  ...
                  ?>  
                        ...
                        if ($oneAction['type'] == 'staging' || $oneAction['type'] == 'stagingToLive'){
                          processQueueTweak($oneAction, 'staging', 'content');
                              ...
                              <?php printDebugLog($oneAction); ?> 
                              <div style="position: absolute;margin-top: -13px;margin-left: 61px;"> <?php if (!empty($oneAction['successMsg'])) { echo $oneAction['successMsg']; } ?>
                </div>
                              <div class="clear-both"></div>
                        ...
                        <?php foreach($statusGroupedActions['error'] as $oneAction){
                            if ($oneAction['type'] == 'staging' || $oneAction['type'] == 'stagingToLive'){
                        	  ...
                        	  <!--For each plugins failed plugins starts-->
                              <?php if($showByDetailedActionGroup){ ?><div class="failure_name"><?php echo $oneAction['name'].'-'.$oneAction['version']; ?></div><?php } ?>
                              <a style="float:right;" class="retry_failed_task needConfirm" historyid="<?php echo $oneAction['historyID']; ?>">Retry</a>
                              <div class="failure_reason<?php if(!$showByDetailedActionGroup){ ?> only<?php } ?>"><?php echo TPLAddErrorHelp($oneAction); ?><a style="float:right;margin-right: 10px;margin-top: -2px;" class="curlVerbose" historyID="<?php echo $oneAction['historyID']; ?>">Curl verbose</a></div> <?php printDebugLog($oneAction); ?>   <div style="position: absolute;margin-top: -13px;margin-left: 61px;"> <?php if (!empty($oneAction['successMsg'])) { echo $oneAction['successMsg']; } ?>
